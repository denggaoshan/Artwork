 ...
 * @Since: Ultra 1.0 This file is part of Ultra Admin Theme HTML package.
 */
...
jQuery(function($) {
	...
	ULTRA_SETTINGS.windowBasedLayout = function() {
		...
		if ($("body").hasClass("chat-open")
			...
			// small window
			$(".page-topbar").addClass("sidebar_shift").removeClass(
					"chat_shift");
			$(".page-sidebar").addClass("collapseit").removeClass("expandit");
			$("#main-content").addClass("sidebar_shift").removeClass(
					"chat_shift");
		...
		$('.page-topbar .sidebar_toggle').on(
				...
				function() {
					...
					if (menuarea.hasClass("collapseit")
							|| menuarea.hasClass("chat_shift")) {
						menuarea.addClass("expandit").removeClass("collapseit")
								.removeClass("chat_shift");
						topbar.removeClass("sidebar_shift").removeClass(
								"chat_shift");
						mainarea.removeClass("sidebar_shift").removeClass(
								"chat_shift");
						chatarea.addClass("hideit").removeClass("showit");
						chatwindow.addClass("hideit").removeClass("showit");
						ULTRA_SETTINGS.mainmenuScroll();
						...
						menuarea.addClass("collapseit").removeClass("expandit")
								.removeClass("chat_shift");
						topbar.addClass("sidebar_shift").removeClass(
								"chat_shift");
						mainarea.addClass("sidebar_shift").removeClass(
								"chat_shift");
						ULTRA_SETTINGS.mainmenuCollapsed();
					}
				});
	...
	ULTRA_SETTINGS.chatApiWindow = function() {
		...
		$('.page-chatapi .user-row')
				.on(
						'click',
						function() {

							var name = $(this).find(".user-info h4 a").html();
							var img = $(this).find(".user-img a img").attr(
									"src");
							var id = $(this).attr("data-user-id");
							var status = $(this).find(".user-info .status")
									.attr("data-status");
							...
							if ($(this).hasClass("active")) {
								...
								if ($(".chatapi-windows #user-window" + id).length) {
									...
									$(".chatapi-windows #user-window" + id)
											.removeClass("minimizeit").show();

								} else {
									var msg = chatformat_msg(
											'Wow! What a Beautiful theme!',
											'receive', name);
									msg += chatformat_msg(
											'Yes! Ultra Admin Theme ;)',
											'sent', 'You');
									var html = "<div class='user-window' id='user-window"
											+ id
											+ "' data-user-id='"
											+ id
											+ "'>";
									html += "<div class='controlbar'><img src='"
											+ img
											+ "' data-user-id='"
											+ id
											+ "' rel='tooltip' data-animate='animated fadeIn' data-toggle='tooltip' data-original-title='"
											+ name
											+ "' data-placement='top' data-color-class='primary'><span class='status "
											+ status
											+ "'><i class='fa fa-circle'></i></span><span class='name'>"
											+ name
											+ "</span><span class='opts'><i class='fa fa-times closeit' data-user-id='"
											+ id
											+ "'></i><i class='fa fa-minus minimizeit' data-user-id='"
											+ id + "'></i></span></div>";
									html += "<div class='chatarea'>" + msg
											+ "</div>";
									html += "<div class='typearea'><input type='text' data-user-id='"
											+ id
		...
		$(document).on(
				'click',
				".chatapi-windows .user-window .controlbar .closeit",
				function(e) {
					var id = $(this).attr("data-user-id");
					$(".chatapi-windows #user-window" + id).hide();
					$(".page-chatapi .user-row#chat_user_" + id).removeClass(
							"active");
				});
		...
		$(document)
				.on(
						'click',
						".chatapi-windows .user-window .controlbar img, .chatapi-windows .user-window .controlbar .minimizeit",
						function(e) {
							var id = $(this).attr("data-user-id");

							if (!$(".chatapi-windows #user-window" + id)
									.hasClass("minimizeit")) {
								$(".chatapi-windows #user-window" + id)
										.addClass("minimizeit");
		...
		$(document).on(
				...
				function(e) {
					if (e.keyCode == 13) {
						...
						$(".chatapi-windows #user-window" + id + " .chatarea")
								.append(msg);
						$(this).val("");
						$(this).focus();
					}
	...
	function chatformat_msg(msg, type, name) {
		...
		var h = d.getHours();
		var m = d.getMinutes();
		return "<div class='chatmsg msg_" + type + "'><span class='name'>"
				+ name + "</span><span class='text'>" + msg
				+ "</span><span class='ts'>" + h + ":" + m + "</span></div>";
	}
		...
		if ($.isFunction($.fn.viewportChecker)) {
			...
			$('.inviewport').viewportChecker({
				callbackFunction : function(elem, action) {
					// setTimeout(function(){
					// elem.html((action == "add") ? 'Callback with 500ms
					// timeout: added class' : 'Callback with 500ms timeout:
					// removed class');
					// },500);
				}
			});
	...
	ULTRA_SETTINGS.sectionBoxActions = function() {
		...
		$('section.box .actions .box_toggle').on(
				'click',
				function() {

					var content = $(this).parent().parent().parent().find(
							".content-body");
	...
	ULTRA_SETTINGS.mainmenuScroll = function() {
		...
		// > in the selector is used to select only immediate elements and not
		// the inner nested elements.
	...
	ULTRA_SETTINGS.mainmenuCollapsed = function() {
		...
		if ($(".page-sidebar.chat_shift #main-menu-wrapper").length > 0
				|| $(".page-sidebar.collapseit #main-menu-wrapper").length > 0) {
			// console.log("collapse menu");
			var topbar = $(".page-topbar").height();
			var windowheight = window.innerHeight;
			var minheight = windowheight - topbar;
			var fullheight = $(".page-container #main-content .wrapper")
					.height();
			...
			$(
					'.page-sidebar.chat_shift #main-menu-wrapper .wraplist, .page-sidebar.collapseit #main-menu-wrapper .wraplist')
					.height(height);
	...
	ULTRA_SETTINGS.mainMenu = function() {
		...
		$("body").click(
				function(e) {
					$(".page-sidebar.collapseit .wraplist li.open .sub-menu")
							.attr("style", "");
					$(".page-sidebar.collapseit .wraplist li.open")
							.removeClass("open");
					$(".page-sidebar.chat_shift .wraplist li.open .sub-menu")
							.attr("style", "");
					$(".page-sidebar.chat_shift .wraplist li.open")
							.removeClass("open");
				});
	...
	ULTRA_SETTINGS.extraFormSettings = function() {
		...
		$('.input-group .form-control').focus(
				function(e) {
					$(this).parent().find(".input-group-addon").addClass(
							"input-focus");
					$(this).parent().find(".input-group-btn").addClass(
							"input-focus");
				});

		$('.input-group .form-control').blur(
				function(e) {
					$(this).parent().find(".input-group-addon").removeClass(
							"input-focus");
					$(this).parent().find(".input-group-btn").removeClass(
							"input-focus");
				});
						...
						.jstree(
								{
									"core" : {
										...
										'data' : {
											'url' : function(node) {
												return node.id === '#' ? 'data/ajax_demo_roots_jstree.json'
														: 'data/ajax_demo_children_jstree.json';
											},
											'data' : function(node) {
												return {
													'id' : node.id
					...
					.ready(
							function() {
								...
								tableElement
										.dataTable({
											...
											fnPreDrawCallback : function() {
												...
												if (!responsiveHelper) {
													// responsiveHelper = new
													// ResponsiveDatatablesHelper(tableElement,
								...
								$('#example_wrapper .dataTables_filter input')
										.addClass("input-medium "); // modify
																	// table
																	// search
																	// input
								$('#example_wrapper .dataTables_length select')
										.addClass("select2-wrapper col-md-12"); // modify
																				// table
																				// per
																				// page
																				// dropdown
								...
								$('#example2 thead tr').each(
										function() {
											this.insertBefore(nCloneTh,
													this.childNodes[0]);
										});

								$('#example2 tbody tr').each(
										function() {
								...
								/*
								 * Initialse DataTables, with no sorting on the
								 * 'details' column
								 */
								var oTable = $('#example2')
										.dataTable(
								...
								$('#example2_wrapper .dataTables_filter input')
										.addClass("input-medium ");
								$('#example2_wrapper .dataTables_length select')
										.addClass("select2-wrapper col-md-12");

								/*
								 * Add event listener for opening and closing
								 * details Note that the indicator for showing
								 * which row is open is not controlled by
										...
										.on(
												...
												function() {
													...
													function fnFormatDetails(
														...
														sOut += '<tr><td>Rendering engine:</td><td>'
																+ aData[1]
																+ ' '
																+ aData[4]
																+ '</td></tr>';
			...
			$("#add_event_form")
					.on(
							...
							function(ev) {
								...
								var $event = $(this).find('.new-event-form'), event_name = $event
										.val();
	...
	ULTRA_SETTINGS.nestableList = function() {
		...
		$("#nestableList-1").on('stop.uk.nestable', function(ev) {
			var serialized = $(this).data('nestable').serialize(), str = '';
		...
		function nestableIterate(items, depth) {
			...
			jQuery.each(items, function(i, obj) {
				str += '[ID: ' + obj.itemId + ']\t'
						+ nestableRepeat('—', depth + 1) + ' ' + obj.item;
				str += '\n';
	...
	ULTRA_SETTINGS.iCheck = function() {
		...
		if ($.isFunction($.fn.iCheck)) {
			...
			var x;
			var colors = [ "-green", "-red", "-yellow", "-blue", "-aero",
					"-orange", "-grey", "-pink", "-purple", "-white" ];
	...
	ULTRA_SETTINGS.formEditors = function() {
			...
			$('.bootstrap-wysihtml5-textarea').wysihtml5({
				toolbar : {
					...
					"lists" : true, // (Un)ordered lists, e.g. Bullets, Numbers.
									// Default true
					"html" : true, // Button which allows you to edit the
									// generated HTML. Default false
					"link" : true, // Button to insert a link. Default true
					"image" : true, // Button to insert an image. Default true,
					"color" : true, // Button to change color of font
					"blockquote" : true, // Blockquote
					"size" : "none" // default: none, other options are xs, sm,
									// lg
				}
			});
			...
			$('.mail-compose-editor').wysihtml5({
				toolbar : {
					"font-styles" : true, // Font styling, e.g. h1, h2, etc.
											// Default true
					"emphasis" : true, // Italics, bold, etc. Default true
					"lists" : false, // (Un)ordered lists, e.g. Bullets,
		...
		if ($.isFunction($.fn.CKEDITOR)) {
			// This code is generally not necessary, but it is here to
			// demonstrate
			// how to customize specific editor instances on the fly. This fits
			// well
			...
			CKEDITOR
					.on(
							...
							function(event) {
								...
								// These editors don't need features like
								// smileys, templates, iframes etc.
								if (element.is('h1', 'h2', 'h3')
										|| element.getAttribute('id') == 'taglist') {
									// Customize the editor configurations on
									// "configLoaded" event,
									// which is fired after the configuration
									// file loading and
									// execution. This makes it possible to
									// change the
									// configurations before the editor
	...
	ULTRA_SETTINGS.customDropZone = function() {
		...
		if ($.isFunction($.fn.dropzone)) {
			...
			var i = 1, $custom_droplist = $("#custom-droptable"), example_dropzone = $(
					"#customDZ")
					.dropzone(
							{
								...
								addedfile : function(file) {
									...
									size = size < 1024 ? (size + " KB")
											: (parseInt(size / 1024, 10) + " MB");
								...
								success : function(file) {
									file.progressBar.removeClass(
											'progress-bar-warning').addClass(
	...
	ULTRA_SETTINGS.otherScripts = function() {
		...
		if ($.isFunction($.fn.inputmask)) {
			$("[data-mask]")
					.each(
							function(i, el) {
								var $this = $(el), mask = $this.data('mask')
										.toString(), opts = {
									numericInput : getValue($this, 'numeric',
											false),
									radixPoint : getValue($this, 'radixPoint',
											''),
									rightAlign : getValue($this,
											'numericAlign', 'left') == 'right'
								}, placeholder = getValue($this, 'placeholder',
										''), is_regex = getValue($this,
										'isRegex', '');
									...
									$.extend(opts,
											{
												autoGroup : true,
												groupSize : 3,
												radixPoint : getValue($this,
														'rad', '.'),
												groupSeparator : getValue(
														$this, 'dec', ',')
											});
								}

								if (mask.toLowerCase() == "currency"
										|| mask.toLowerCase() == "rcurrency") {
			...
			$(".slider")
					.each(
							function(i, el) {
								...
								orientation = getValue($this, 'vertical', 0) != 0 ? 'vertical'
										: 'horizontal',

								prefix = getValue($this, 'prefix', ''), postfix = getValue(
										$this, 'postfix', ''),

								fill = getValue($this, 'fill', ''), $fill = $(fill),

								step = getValue($this, 'step', 1), value = getValue(
										$this, 'value', 5), min = getValue(
										$this, 'min', 0), max = getValue($this,
										'max', 100), min_val = getValue($this,
										'min-val', 10), max_val = getValue(
										$this, 'max-val', 90),

								is_range = $this.is('[data-min-val]')
										|| $this.is('[data-max-val]'),
		...
		function refreshSwatch() {
			var red = $("#slider-red").slider("value"), green = $(
					"#slider-green").slider("value"), blue = $("#slider-blue")
					.slider("value"), hex = hexFromRGB(red, green, blue);
		...
		if ($.isFunction($.fn.tagsinput)) {
			...
			var i = -1, colors = [ 'primary', 'info', 'warning', 'success' ];
		...
		if ($.isFunction($.fn.typeahead)) {
			...
			$('#typeahead-4')
					.typeahead(
							...
							'typeahead:opened',
							function() {
								$(this).data('ttTypeahead').dropdown.$menu
	...
	ULTRA_SETTINGS.ultraWidgetWeather = function() {
		...
		/* notification widget */
		/*
		 * var wid = $(".wid-weather"); var notif_widget =
		 * $(".notification-widget").height();
		 * $('.notification-widget').height(notif_widget).perfectScrollbar({suppressScrollX:
		 * true});
		 * 
		 * $('.wid-weather').each( function () { var days =
		 * $(this).find(".weekdays"); var today = $(this).find(".today");
		 * 
		 * var height = days.height(); if(days.height() < today.height()){
		 * height = today.height(); }
		 * 
		 * days.height(height); today.height(height); });
		 */
				...
				.on(
						...
						function(e) {
							if (e.keyCode == 13) {
								...
								var msg = $(this).val();
								var msg = '<li><input type="checkbox" id="task-'
										+ i
										+ '" class="icheck-minimal-white todo-task"><label class="icheck-label form-label" for="task-'
										+ i + '">' + msg + '</label></li>';
								$(this).parent().parent().find(
										".wid-all-tasks ul").append(msg);
								$(this).val("");
								$(this).focus();
						...
						.vectorMap(
								{
									...
									onRegionSelected : function(event, code,
											isSelected, selectedRegions) {
										// console.log('region-select', code,
										// isSelected, selectedRegions);
										if (window.localStorage) {
											window.localStorage
													.setItem(
	...
	ULTRA_SETTINGS.widgetSparklineChart = function() {
		...
		if ($.isFunction($.fn.sparkline)) {
			...
			$('.wid_dynamicbar').sparkline(
					[ 8.4, 9, 8.8, 8, 9.5, 9.2, 9.9, 9, 9, 8, 7, 8, 9, 8, 7, 9,
							9, 9.5, 8, 9.5, 9.8 ], {
						type : 'bar',
						barColor : '#f5f5f5',
						height : '60',
			...
			$('.wid_linesparkline').sparkline(
					[ 2000, 3454, 5454, 2323, 3432, 4656, 2897, 3545, 4232,
							4656, 2897, 3545, 4232, 5434, 4656, 3567, 4878,
							3676, 3787 ], {
						type : 'line',
						width : '100%',
						height : '60',
			...
			$('.wid_compositebar').sparkline(
					[ 4, 6, 7, 7, 4, 3, 2, 4, 6, 7, 7, 8, 8, 4, 4, 3, 1, 4, 6,
							5, 9 ], {
						type : 'bar',
						barColor : '#f5f5f5',
						height : '60',
			...
			$('.wid_compositebar').sparkline(
					[ 4, 1, 5, 7, 9, 9, 8, 8, 4, 7, 8, 4, 7, 9, 9, 8, 8, 4, 2,
							5, 6, 7 ], {
						composite : true,
